...
app_image = (
    ...
    .pip_install("numpy", "scipy", "pyscf", "openfermion")
)

app = modal.App("nh3-confinement", image=app_image)
...
@app.function(timeout=86400, cpu=4.0, memory=16384)
def run_experiment(run_uccsd: bool, dry_run: bool):
    log("=" * 65)
    log("  SymUCCSD DLA Confinement — NH₃ / STO-3G")
    log("  da Silva & Santos (2025) [Modal]")
    ...
    e, nfev, hist = run_vqe(sym_gens, H_sparse, hf_state, e_fci,
                             'SymUCCSD', maxiter=15000, dry_run=dry_run)
    ...
    if run_uccsd:
        ...
        e, nfev, hist = run_vqe(uccsd_gens, H_sparse, hf_state, e_fci,
                                 'UCCSD', maxiter=20000, dry_run=dry_run)
...
def main(
    run_uccsd: bool = False,
    dry_run: bool = False,
):
    print("🚀 Submitting to Modal...")
    out = run_experiment.remote(run_uccsd, dry_run)
    ...
    print("\n" + "=" * 65)
    print("  RESULTS — NH₃/STO-3G, 16 qubits, no frozen core")
    if dry_run:
        print("  ⚠  DRY RUN — energies are NOT converged")
    print("=" * 65)
    print(f"{'Method':<20} {'Params':>7} {'Energy (Ha)':>18} {'ΔE (mHa)':>10}")
    print("-" * 59)
    print(f"{'HF':<20} {'—':>7} {e_hf:>18.10f} {(e_hf-e_fci)*1000:>10.3f}")

    for label in ['SymUCCSD', 'UCCSD']:
        if label in results:
            e_val, _, np_ = results[label]
            print(f"{label:<20} {np_:>7d} {e_val:>18.10f} "
                  f"{(e_val-e_fci)*1000:>10.3f}")

    if 'UCCSD' not in results:
        print(f"{'UCCSD (He et al.)':<20} {'135':>7} {'—':>18} {'0.109':>10}")

    print(f"{'FCI':<20} {'—':>7} {e_fci:>18.10f} {'0.000':>10}")
    print("-" * 59)
    ...
    json_histories = {}
    for label, hist in out.get("histories", {}).items():
    ...
    json_data = {
        ...
        "molecule": "NH3",
        "basis": "sto-3g",
        "n_qubits": out.get("n_qubits", 16),
        "n_electrons": out.get("n_electrons", 10),
        "frozen_core": False,
        "optimizer": "BFGS",
        "gtol": 1e-4,
        "e_hf": e_hf,
        "e_ccsd": out.get("e_ccsd"),
        "e_fci": e_fci,
